...
static int SPI1_SlaveTxData[256];
static int SPI1_SlaveRxData[256];
...
static irqreturn_t nuc980_spi1_irq(int irq, void *dev)
{
	...
	if (status & RXTHIF) {
		while(!(__raw_readl(hw->regs + REG_STATUS) & 0x100)) {//RXEMPTY
			SPI1_SlaveRxData[ReceivedCnt++] = __raw_readl(hw->regs + REG_RX);

			if (!(__raw_readl(hw->regs + REG_STATUS) & 0x20000)) {//TXFULL
				__raw_writel(SPI1_SlaveTxData[TransmittedCnt++], hw->regs + REG_TX);
...
static int nuc980_spi1_slave_probe(struct platform_device *pdev)
{
	...
	*/
	...
	kthread_run((int (*)(void *))SPI1_Slave_Thread_TXRX, hw, "SPI1_SLAVE_THread_TXRX");
