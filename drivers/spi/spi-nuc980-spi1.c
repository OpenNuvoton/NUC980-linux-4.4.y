...
#if defined(CONFIG_SPI_NUC980_SPI1_NO_PDMA)
...
static int nuc980_spi1_txrx(struct spi_device *spi, struct spi_transfer *t)
{
...
#if defined(CONFIG_SPI_NUC980_SPI1_PDMA)
	spi1_src_mem_p.size = t->len;
	spi1_src_mem_p.vir_addr = (unsigned int)dma_alloc_writecombine(NULL,
	                          PAGE_ALIGN(spi1_src_mem_p.size),
	                          &spi1_src_mem_p.phy_addr,
	                          GFP_KERNEL);

	if (t->tx_buf)
		memcpy((void *)spi1_src_mem_p.vir_addr, (const void *)t->tx_buf, t->len);

	spi1_dest_mem_p.size = t->len;
	spi1_dest_mem_p.vir_addr = (unsigned int)dma_alloc_writecombine(NULL,
	                           PAGE_ALIGN(spi1_dest_mem_p.size),
	                           &spi1_dest_mem_p.phy_addr,
	                           GFP_KERNEL);
	...
	if (t->rx_buf) {
		...
		pdma_rx->sgrx[0].dma_address=spi1_dest_mem_p.phy_addr;
		pdma_rx->sgrx[0].length=spi1_dest_mem_p.size;
		...
		pdma_rx->rxdesc->callback = spi1_nuc980_slave_dma_callback;
		pdma_rx->rxdesc->callback_param = &spi1_dma_slave_done;
	...
	pdma_tx->sgtx[0].dma_address = spi1_src_mem_p.phy_addr;;
	pdma_tx->sgtx[0].length = spi1_src_mem_p.size;
	...
	if (!t->rx_buf) {
		pdma_tx->txdesc->callback = spi1_nuc980_slave_dma_callback;
		pdma_tx->txdesc->callback_param = &spi1_dma_slave_done;
	} else {
	...
	if (t->rx_buf)
		memcpy((void *)t->rx_buf, (const void *)spi1_dest_mem_p.vir_addr, t->len);

	dma_free_writecombine(NULL, PAGE_ALIGN(spi1_src_mem_p.size), (void *)spi1_src_mem_p.vir_addr, spi1_src_mem_p.phy_addr);
	dma_free_writecombine(NULL, PAGE_ALIGN(spi1_dest_mem_p.size), (void *)spi1_dest_mem_p.vir_addr, spi1_dest_mem_p.phy_addr);
